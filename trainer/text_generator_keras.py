...
def train_model(train_file='input.txt', job_dir='./tmp/', embeds_file='glove.840B.300d-char.txt', **args):
    ...
    for iteration in range(1, 20):
        ...
        f3.close()
        store_in_gcs('output/char-embeddings.txt', 'char-embeddings.txt', job_dir)
...
def store_in_gcs(filepath, gcs_name, job_dir):
    with file_io.FileIO(filepath, mode='r') as input_f:
        with file_io.FileIO(job_dir + '/' + gcs_name, mode='w+') as output_f:
            output_f.write(input_f.read())

class BatchLossLogger(Callback):
    def __init__(self,iteration,start_time,log_writer):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        if batch % 50 == 0:
            self.log_writer.writerow([self.iteration, batch,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      '--embeds-file',
      help='GCS or local paths to pretrained char embeddings data',
      required=False
    )
